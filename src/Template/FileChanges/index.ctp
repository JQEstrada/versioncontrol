...
<div class="fileChanges index large-9 medium-8 columns content">
    <h3><?= __('File Changes') ?></h3>
    <table cellpadding="0" cellspacing="0">
        ...
        <tbody>
            <?php foreach ($fileChanges as $fileChange): ?>
            <tr>
                ...
                <td class="actions">
                    <?= $this->Html->link(__('View'), ['action' => 'view', $fileChange->id]) ?>
                    <?= $this->Html->link(__('Edit'), ['action' => 'edit', $fileChange->id]) ?>
                    <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $fileChange->id], ['confirm' => __('Are you sure you want to delete # {0}?', $fileChange->id)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
