<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Form->postLink(
                __('Delete'),
                ['action' => 'delete', $fileChange->id],
                ['confirm' => __('Are you sure you want to delete # {0}?', $fileChange->id)]
            )
        ?></li>
...
<div class="fileChanges form large-9 medium-8 columns content">
    <?= $this->Form->create($fileChange) ?>
    <fieldset>
        <legend><?= __('Edit File Change') ?></legend>
        <?php
